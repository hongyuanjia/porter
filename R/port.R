...
#' Validate generated function symbols against a loaded dynamic library
#'
#' @param port A `dynport` object created by [port()].
#' @param lib A library handle returned by `rdyncall::dynload()`, or a library
#'        name/path accepted by `rdyncall::dynload()`.
#'
#' @return A data frame with symbol validation status.
#' @export
port_validate_symbols <- function(port, lib) {
    if (!inherits(port, "dynport")) {
        stop("Argument 'port' should be a dynport object.")
    }
    if (!requireNamespace("rdyncall", quietly = TRUE)) {
        stop("Package 'rdyncall' is required for symbol validation.")
    }

    close <- FALSE
    if (is.character(lib)) {
        lib <- rdyncall::dynload(lib)
        if (is.null(lib)) stop("Failed to load dynamic library.")
        close <- TRUE
        on.exit(rdyncall::dynunload(lib), add = TRUE)
    }

    funs <- port_get(port, "Function")
    ...
    if (!nrow(symbols)) {
        symbols$status <- character()
        return(symbols)
    }

    listed <- try(rdyncall::dynlist(lib), silent = TRUE)
    if (!inherits(listed, "try-error")) {
        symbols$status <- ifelse(symbols$name %in% listed, "available", "missing")
    } else {
        symbols$status <- "unknown"
    }

    needs_probe <- symbols$status != "available"
    if (any(needs_probe)) {
        ok <- vapply(symbols$name[needs_probe], function(name) {
            !is.null(tryCatch(rdyncall::dynsym(lib, name), error = function(e) NULL))
        }, logical(1))
        idx <- which(needs_probe)
        symbols$status[idx[ok]] <- "available"
        unknown <- idx[!ok & symbols$status[idx] == "unknown"]
        symbols$status[unknown] <- "missing"
    }

    if (close) invisible(NULL)
    symbols
}
